...
class SnakeGameAI:
    ...
    def play_step(self, action,action2):
        ...
        reward2 = 0
        game_over = False
        if  self.is_collision3():
            game_over = True
            reward = -10
            return reward, game_over, self.score,reward, game_over, self.score2
        
        if self.is_collision1() or self.frame_iteration > 100*len(self.snake) :
            game_over = True
            reward = -10
            return reward, game_over, self.score,0, False, self.score2


        if self.is_collision2() or self.frame_iteration2 > 100*len(self.snake2):
            game_over = True
            reward = -10
    ...
    def is_collision1(self, pt=None, pt2=None):
        if pt is None:
            pt = self.head
        if pt2 is None:
            pt2 = self.head2
    ...
    def is_collision2(self, pt=None, pt2=None):
        if pt is None:
            pt = self.head
        if pt2 is None:
            pt2 = self.head2
        # hits boundary
        if pt2.x > self.w - BLOCK_SIZE or pt2.x < 0 or pt2.y > self.h - BLOCK_SIZE or pt2.y < 0:
            return True
        # hits itself
        if pt2 in self.snake2[1:] or pt in self.snake2[1:] or pt2 in self.snake[1:] or pt2 in self.snake[1:]:
            return True
